...
public class AccountServiceImpl implements AccountService {
    ...
    public Account detailAccountByUserName(String username) {
        return accountRepository.detailAccountByUsername(username);
    }

    @Override
    public UserDetailCustom loadCustomsAccountByIdAccount(Long idAccount) {
        Optional<Account> account = accountRepository.findById(idAccount);
        if(!account.isPresent()){
            throw new RuntimeException("Account not exit");
        }
